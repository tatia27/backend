...
const authHeader = {
  Authorization:
    "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2NjUzNDY4ZTZhMDkxMTBhZTA4OWRkMDciLCJyb2xlIjoiY29tcGFueSIsImlhdCI6MTcxNjczNjEwMiwiZXhwIjoxNzI1Mzc2MTAyfQ.CRAHpQwjxnZVY72jqb1LxmuyVLXoGpoBoRIm4uqK94o",
};
...
describe("Test cases to add correct internships in collection", () => {
  ...
  it("Test to add correctInsternship internship in collection", async () => {
    ...
    const respone = await request(app)
      .post(`/v1/internships/${id}`)
      .set("Authorization", authHeader.Authorization)
      .send(correctInsternship);
...
describe("Test cases to add uncorrect internships in collection", () => {
  ...
  it("Test to add falseConditionsInsternship internship in collection", async () => {
    const respone = await request(app)
      .post(`/v1/internships/${id}`)
      .set("Authorization", authHeader.Authorization)
      .send(falseConditionsInsternship);
    expect(respone.status).toBe(400);
  });
});
