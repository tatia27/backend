...
export const login = async (req, res) => {
  ...
  if (!email || !password) {
    return res
      ...
      .json({ message: "Please fill full form!" });
  }
  ...
  if (company) {
    ...
    if (!isCompanyPasswordCorrect) {
      return res
        .status(ERRORS.WRONG_DATA.CODE)
        .json({ message: ERRORS.WRONG_DATA.TITLE });
    }
    ...
    return res
      .status(ERRORS.USER_NOT_FOUND.CODE)
      .json({ error: ERRORS.USER_NOT_FOUND.TITLE });
  }
};
...
export const isAuth = async (req, res) => {
  ...
  if (decoded.role === "intern") {
    ...
    if (intern) {
      return res
        ...
        .json({ role: intern.role, id: intern._id });
    }
    return res
      .status(ERRORS.USER_NOT_FOUND.CODE)
      .json({ message: ERRORS.USER_NOT_FOUND.TITLE });
  }
  ...
  if (decoded.role === "company") {
    ...
    if (company) {
      return res
        .status(HTTP_CODES.SUCCESS)
        .json({ role: company.role, id: company._id });
    }
  ...
  return res
    .status(ERRORS.NOT_AUTHORIZED.CODE)
    .json({ message: ERRORS.NOT_AUTHORIZED.TITLE });
};
