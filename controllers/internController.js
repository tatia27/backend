  ...
  try {
    ...
    if (isEmail) {
      return res
        ...
        .json({ message: ERRORS.CONFLICT.TITLE });
    }
...
export const updateIntern = async (req, res, next) => {
  try {
    ...
    res.status(HTTP_CODES.SUCCESS).json({
      firstName: updateIntern.firstName,
      middleName: updateIntern.middleName,
      lastName: updateIntern.lastName,
      email: updateIntern.email,
    });
  } catch (err) {
    next(err);
  }
...
export const getFavoritesInternships = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res
        .status(ERRORS.INTERN_NOT_FOUND.CODE)
        .json({ message: ERRORS.INTERN_NOT_FOUND.TITLE });
    }
