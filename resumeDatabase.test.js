...
describe("Test cases to add correct resume in collection", () => {
    ...
    it("Test to add correctResume internship in collection", async () => {
        ...
        const respone = await request(app).put(`/v1/intern/${id}/resume`).set("Authorization", authHeader.Authorization).send(correctResume)
...
describe("Test cases to add uncorrect internships in collection", () => {
    ...
    it("Test to add falseAgeResume resume in collection", async () => {
        const respone = await request(app).put(`/v1/intern/${id}/resume`).set("Authorization", authHeader.Authorization).send(falseAgeResume)
        expect(respone.status).toBe(400)
    })

    it("Test to add falseLocationResume resume in collection", async () => {
        const respone = await request(app).put(`/v1/intern/${id}/resume`).set("Authorization", authHeader.Authorization).send(falseLocationResume)
        expect(respone.status).toBe(400)
    })

    it("Test to add falseInstitutionResume resume in collection", async () => {
        const respone = await request(app).put(`/v1/intern/${id}/resume`).set("Authorization", authHeader.Authorization).send(falseInstitutionResume)
        expect(respone.status).toBe(400)
    })

    it("Test to add falseSpecializationResume resume in collection", async () => {
        const respone = await request(app).put(`/v1/intern/${id}/resume`).set("Authorization", authHeader.Authorization).send(falseSpecializationResume)
        expect(respone.status).toBe(400)
    })

    it("Test to add falseHardResume resume in collection", async () => {
        const respone = await request(app).put(`/v1/intern/${id}/resume`).set("Authorization", authHeader.Authorization).send(falseHardResume)
        expect(respone.status).toBe(400)
    })

    it("Test to add falseSoftResume resume in collection", async () => {
        const respone = await request(app).put(`/v1/intern/${id}/resume`).set("Authorization", authHeader.Authorization).send(falseSoftResume)
        expect(respone.status).toBe(400)
    })

    it("Test to add falseLevelResume resume in collection", async () => {
        const respone = await request(app).put(`/v1/intern/${id}/resume`).set("Authorization", authHeader.Authorization).send(falseLevelResume)
        expect(respone.status).toBe(400)
    })
})
